...
contract SortingCenter is IBaseWarehouse, BaseWarehouseStorage, OwnableUpgradeable {
    ...
    event SortingCenterAdded(string name, int256 lat, int256 lon);
    ...
    function initialize(uint256 _id, address _supplyChain, address _productRegistry, address _newOwner, string memory _name, int256 _lat, int256 _lon) external initializer {
        ...
        warehouseType = WarehouseType.SortingCenter;
        ...
        emit SortingCenterAdded(name, lat, lon);
    }
    ...
    function takeProduct(uint256 _deliveryId, address _courier, uint256 _weightGram, uint256 _volumeCm3) external onlyOwner {
