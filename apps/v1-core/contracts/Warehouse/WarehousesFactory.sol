...
contract SupplyChainWarehouseFactory is Ownable {
    ...
    function createWarehouse(WarehouseType _warehouseType, string memory _name, int256 _lat, int256 _lon) external returns (address) {
        require(implementations[_warehouseType] != address(0), "Implementation not set");
        
        address clone = implementations[_warehouseType].clone();
        
        IBaseWarehouse(clone).initialize(++warehouseCounter, supplyChain, productRegistry, msg.sender, _name, _lat, _lon);
