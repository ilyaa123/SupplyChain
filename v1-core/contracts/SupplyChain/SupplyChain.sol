...
contract SupplyChain is Ownable, ISupplyChain, SupplyChainOrders, SupplyChainDeliveries, SupplyChainDeliveryCalculator, SupplyChainApprovedWarehouses, DisnanceConsumer {
    ...
    function _payForCouriers(DeliveryLeg[] memory legs, uint256 deliveryCost) private returns (bool) {
        ...
        for (uint256 i = 0; i < legs.length; i++) {
            ...
            require(success, "Courier payment failed");
        }
    ...
    function completeDeliveryOrder(uint256 _deliveryId) external {
        DeliveryProduct memory deliveryProduct = deliveryProducts[_deliveryId];
    ...
    /// @dev Запрос доставyки товара (создается из ShopContract)
    function createDeliverRequest(uint256 _productId, uint256 _price, uint256 _fromWarehouseId, uint256 _toWarehouseId, address _buyer) external {
        (int256 latFrom, int256 lonFrom) = approvedWarehouses[_fromWarehouseId].location();
        (int256 latTo, int256 lonTo) = approvedWarehouses[_toWarehouseId].location();

        uint256 count = approvedWarehouses[_fromWarehouseId].balanceOfProduct(msg.sender, _productId);
    
        require(approvedWarehouses[_fromWarehouseId].warehouseType() == WarehouseType.Warehouse, "Not a Warehouse");
        ...
        uint256 orderId = _createOrder(msg.sender, _buyer, productIds, prices, _fromWarehouseId, _toWarehouseId);

        uint256 deliveryId = _createDelivery(orderId, _productId, _toWarehouseId);

        IWarehouse(address(approvedWarehouses[_fromWarehouseId])).reserveForDelivery(deliveryId, _productId, _toWarehouseId);

        _requestOrderDistance(orderId, args, encryptedSecretsUrls, 200000);
    }

    /// @dev Оплата доставки (в ETH)
